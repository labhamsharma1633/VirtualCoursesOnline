...
function EditCourses() {
  ...
  const [frontedImage,setFrontendImage]=useState(img);
      ...
      <div className='bg-gray-50 p-6 rounded-md'>
        ...
        <form action="" className='space-y-6' onSubmit={(e)=>e.preventDefault()}>
          ...
          <div>
            ...
            <input id='subtitle' type="text" className='w-full border px-4 py-2 rounded-md' placeholder='Subtitle' onChange={(e)=>setSubTitle(e.target.value)} value={subTitle} />
          </div>
          <div>
            <label htmlFor="des"className='block text-sm font-medium text-gray-700 mb-1'>Description</label>
            <textarea id='description' type="text" className='w-full border px-4 py-2 rounded-md h-24 resize-none' placeholder='Course Description' onChange={(e)=>setDescription(e.target.value)} value={description}></textarea>
          </div>
          <div className='flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0'>
            {/* for category */}
            <div className='flex-1'>
              <label htmlFor="level" className='block text-sm font-medium text-gray 700 mb-1'>Course Category</label>
          ...
          <div className='flex items-center justify-start gap-[15px]'>
            <button className='bg-[#e9e8e8] hover:bg-red-200 text-black border border-black cursor-pointer px-4 py-2-rounded-md' onClick={()=>navigate("/courses")}>
              Cancel

            </button>
            <button className='bg-black text-white px-7 py-2 rounded-md hover:bg-gray-500 cursor-pointer' onClick={handleEditCourse}>
